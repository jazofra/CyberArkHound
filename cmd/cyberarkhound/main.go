...
func main() {
	...
	// Pre-allocate to max possible size to avoid re-allocations (~3x speedup)
	...
	if *includeActivity && len(accounts) > 0 {
		...
		// Pre-allocate map to avoid resizing
		accountActivities = make(map[string][]models.AccountActivity, len(accounts))
